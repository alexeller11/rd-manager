...
def build_prospect_diagnosis(payload: dict) -> dict:
    ...
    if not website:
        ...
        opportunities.append("Criar uma base de captação previsível com landing pages e campanhas.")
        sales_angles.append("Hoje a operação parece depender mais de esforço comercial do que de um funil previsível.")
        action_plan.append("Mapear canais atuais de entrada e estruturar uma jornada mínima de captação.")

    if website:
        weak_points.append("Mesmo com site, a empresa pode não estar convertendo tráfego em oportunidades reais.")
    ...
    if city:
        sales_angles.append(f"Há espaço para posicionar a marca com mais força em {city} por meio de campanhas e presença digital organizada.")
    ...
    weak_points.append("Sem segmentação, a comunicação tende a ser genérica e a conversão fica menor.")
    ...
    proposal = {
        "diagnostico": executive_summary,
        "pitch": "Implementação de estrutura de marketing previsível com foco em geração de demanda, automação e leitura de performance.",
        "entregas": [
            "Mapeamento de captação e pontos de conversão",
            "Estruturação de landing pages por oferta",
            "Criação de segmentações e fluxos automáticos",
            "Calendário de campanhas e acompanhamento mensal",
        ],
    }
