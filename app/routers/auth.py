...
async def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends()):
    user = await db_fetchone("SELECT id, username, password_hash, is_admin FROM users WHERE username = $1", form_data.username)
...
@router.post("/logout")
async def logout():
    # No server-side logout needed for JWT, client just discards token
    return {"ok": True}
