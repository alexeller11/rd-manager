...
async def sync_client(client_id: int):
    ...
    # Contatos (para detectar total e canais de origem)
    data, st, err = await rd_get(token, "/platform/contacts", {"page": 1, "page_size": 50})
    if st == 200 and data:
        contacts = safe_list(data, "contacts", "items")
        total = safe_int(data.get("total"))
        if total > snap["total_leads"]:
            snap["total_leads"] = total
        sources = {}
        for c in contacts:
            src = c.get("traffic_source") or c.get("source") or "Direto"
            sources[src] = sources.get(src, 0) + 1
        if sources:
            total_s = sum(sources.values()) or 1
            snap["channels"] = [
                {"source": k, "count": v, "percentage": round(v / total_s * 100, 1)}
...
async def analyze_marketing(req: AnalysisRequest):
    ...
    prompt = f"Analise o desempenho de marketing e engajamento desta empresa.\n\n{context}"
